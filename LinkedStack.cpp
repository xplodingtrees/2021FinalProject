...
LinkedStack<ItemType>::LinkedStack(const LinkedStack<ItemType> &aStack) {
    ...
    if (origChainPtr == nullptr) //check if empty
        ...
        while(origChainPtr != nullptr){ //copy remaining nodes
            ...
            shared_ptr<Node<ItemType>> newNodePtr = make_shared<Node<ItemType>>(nextItem);//new Node<ItemType>(nextItem); //create a new node containing the next item
...
bool LinkedStack<ItemType>::push(const ItemType &newEntry) {
    shared_ptr<Node<ItemType>> newNodePtr = make_shared<Node<ItemType>>(newEntry, topPtr); //new Node<ItemType>(newEntry, topPtr); //create new node with entry, set to top
